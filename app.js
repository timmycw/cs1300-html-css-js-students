...
const updatePage = async () => {
  ...
  const filtered = fruitsArray.filter(x => {return x.nutritions.sugar > 10});
  const filteredFruits = filtered.map(x => {return x.name});

  // TODO: Create a new HTML element to display your data 
  
  const sugarFruits = document.createElement('div');
  sugarFruits.innerHTML = filteredFruits;

  // TODO: Append your new element to the page
  gallery.append(sugarFruits);
}
